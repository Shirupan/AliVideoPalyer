...
public class PinchImageView extends AppCompatImageView {
    ...
    public Matrix getInnerMatrix(Matrix matrix) {
        ...
        if (isReady()) {
            ...
            /* float radio = (float) getDrawable().getIntrinsicWidth() / (float) getDrawable().getIntrinsicHeight();
            float dstHeight = 0;
            if (radio != 0) {
                dstHeight = getWidth() / radio;
            }
           if (dstHeight != 0 && dstHeight > getHeight()) {
                //高度超长的图片，原比例将控件填满，可滑动
                float scale = (float) getWidth() / (float) getDrawable().getIntrinsicWidth();
