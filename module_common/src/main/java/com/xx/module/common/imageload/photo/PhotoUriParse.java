...
public class PhotoUriParse {
    private static final String TAG = PhotoUriParse.class.getName();
    ...
    public static Uri getUriForFile(Context context, File file) {
        return FileProvider.getUriForFile(context, PhotoConstant.getFileProviderName(context), file);
    }
    ...
    public static String parseOwnUri(Context context, Uri uri) {
        if (uri == null) return null;
        String path;
        if (TextUtils.equals(uri.getAuthority(), PhotoConstant.getFileProviderName(context))) {
    ...
    public static String getFilePathWithUri(Uri uri, Activity activity) {
        ...
        if (TextUtils.isEmpty(picturePath))
            return "";
        if (!PhotoImageFiles.checkMimeType(activity, PhotoImageFiles.getMimeType(activity, uri)))
            return "";
        return picturePath;
    }
    ...
    public static File getFileWithUri(Uri uri, Activity activity) {
        ...
        if (ContentResolver.SCHEME_CONTENT.equals(scheme)) {
            ...
            } else if (TextUtils.equals(uri.getAuthority(), PhotoConstant.getFileProviderName(activity))) {
    ...
    public static String getFilePathWithDocumentsUri(Uri uri, Activity activity) {
        ...
        if (ContentResolver.SCHEME_CONTENT.equals(uri.getScheme()) && uri.getPath().contains("document")) {
            File tempFile = PhotoImageFiles.getTempFile(activity, uri);
            try {
                PhotoImageFiles.inputStreamToFile(activity.getContentResolver().openInputStream(uri), tempFile);
